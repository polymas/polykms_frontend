...
# API基础URL（可选）
# 生产模式必须设置，开发模式不设置时走vite代理到 http://localhost:8866
# 可以设置完整的URL（如：https://api.polyking.site）或IP地址（如：http://43.156.247.73:8866）
VITE_API_BASE_URL=
```
...
| `VITE_API_BASE_URL` | 否 | - | API服务器地址（IP或域名）。开发模式不设置时走vite代理，生产模式必须设置 |
